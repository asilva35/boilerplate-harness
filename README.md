...
## Phase 28: Chat History — Save, Pin, and Export

**Key concept:** distinct from agent memory (Phase 16 - what the *agent* recalls between sessions) - this is the *user* browsing, naming, pinning, and exporting their own past conversations. Depends on Phase 20 (without multi-session, there's only ever "the" conversation to browse). No Go module to migrate: Go is a single-turn process with no notion of saved conversations to come back to.

- `history/types.ts` (new): `ChatSummary` (id/title/pinned/userId/role/profile/updatedAt/messageCount/lastMessage) and `ChatRecord` (a summary plus the full `Message[]`) - the shapes the store and the REST routes below share.
- `history/store.ts` (new): `ChatHistoryStore`, a disk-backed archive under `.harness/chats/` - one `<id>.json` per chat plus an `index.json` for fast listing, same two-tier shape as `memory/session-files.ts`'s `Store` and for the same reason (listing shouldn't require reading every full transcript off disk). `upsert()` is best-effort and a silent no-op for an empty message list or an unsafe id; `update()` (rename/pin) throws `ConfigError` for an id it's never seen, which the route below turns into a 404.
- **Security note worth calling out:** a chat's `id` is the same client-controlled `?session=` value `server.ts` has accepted since Phase 20 - fine as a `Map` key, but this phase is the first place it becomes part of a filesystem path. `isSafeChatId()` allowlists `[A-Za-z0-9_-]` and rejects anything else outright (no sanitizing/escaping) before it ever reaches `readFile`/`writeFile`, closing off a `?session=../../etc/passwd`-style path-traversal attempt. Real ids (`crypto.randomUUID()`, from `useHarnessSocket.ts`) always pass; this only affects someone hand-crafting a session id.
- `history/export.ts` (new): `toMarkdown(record)` - a full transcript, unlike `messages.ts`'s `summarizeBlock` (a one-line preview for `/history` and the Phase 27 dashboard). The JSON half of the export endpoint needs no helper, just `JSON.stringify`.
- `server.ts`: `chatHistory.upsert()` runs in the `finally` block right after every turn (success or error) - the primary write path - and once more per session in the `SIGINT` handler as a safety net for the one case that skips it (a session whose last mutation was a command like `/clear`, not a `send()`). Three new REST routes: `GET /api/chats` (list), `PATCH /api/chats/:id` (rename/pin, body `{title?, pinned?}`), `GET /api/chats/:id/export?format=md|json` (download, `Content-Disposition: attachment`). The `createServer` callback had to stop 404-ing every non-`GET` method up front to make room for `PATCH`.
- `web-app/`: new `components/ChatHistoryPage.tsx` (`/chats`) - list of saved chats sorted pinned-first then most-recent, click-to-rename title (a new shadcn `Input`, this project's first), a pin toggle, and two export links per chat (plain `<a href>` tags with the REST URL, letting the browser's own download handling do the work instead of a fetch+blob dance). "Chats" nav links added to both `ChatPage` and `DashboardPage` headers alongside the existing "Dashboard" link.

**Verified for real, not just typechecked:** 207 backend tests (14 new: 11 for `ChatHistoryStore` including the path-traversal-id case, 3 for `toMarkdown`), `npm run build` (web-app) clean. Live smoke test: `server.ts` started on a scratch port, a real WebSocket turn through OpenRouter, then the full REST surface end-to-end - `GET /api/chats` showed the archived chat with an auto-picked title (truncated from the first user message) right after the turn completed; `PATCH` renamed it and pinned it; a follow-up `GET /api/chats` confirmed both stuck; `GET .../export?format=md` returned a readable transcript with the correct `Content-Disposition` filename; `GET .../export?format=json` round-tripped the full record; an unknown id 404'd on both `PATCH` and export; and `?session=../../etc/passwd` requested via `/export` also 404'd (rejected by `isSafeChatId()`, never reaching the filesystem) rather than leaking a file outside `.harness/chats/`.

Try it: chat for a bit, then open `/chats` - your conversation should already be listed (titled from your first message). Rename it, pin it, click "Export .md", and confirm the downloaded file reads as a clean transcript.

## Phase 29: Multimodal Support (Images)

**Key concept:** `Block` only had `text`/`tool_use`/`tool_result` - for the model to "see" something (a photo of a broken window, a floor plan, a screenshot), it needs a fourth variant that travels as an image instead of text. No Go module to migrate: the original project's `internal/api/types.go` never had one either.

- `provider/types.ts`: `Block` gains `{ type: "image"; mediaType: string; data: string }` - `data` is raw base64, no `data:<mediaType>;base64,` prefix (each provider's own translation layer adds whatever wrapper its SDK needs). Neither provider's model ever emits this back in a `Response` - same as `tool_result`, it's a caller-only variant.
- `anthropic.ts` / `openrouter.ts`: translate it to each SDK's own image shape - Anthropic's `image` content block (`source: {type: "base64", media_type, data}`), OpenAI-compatible's `image_url` with a full data URI. `openrouter.ts`'s `toOpenAIMessages()` only switches a user message's `content` from a plain string to an array of parts when an image is actually present, so every pre-Phase-29 request stays byte-for-byte identical.
- `agent.ts`: `send()` gains an optional second parameter, `images?: {mediaType, data}[]` - additive, every existing caller (`index.ts`, `tui.tsx`, `ResearchSubagent`) keeps sending none and gets the exact old single-text-block message. An empty prompt is valid as long as at least one image is attached (the "photo with no caption" case).
- `context/compactor.ts`: `estimateTokens()` now counts an image's base64 length toward the token-threshold heuristic (a real cost the model actually pays for), but `renderTranscript()` - which feeds the `Summarize` strategy's own request to the model - substitutes a `[image attached: <mediaType>]` placeholder instead of the raw bytes, so summarizing old history never blows up the summarization call itself with a payload it can't act on anyway (the image is gone from context once that message gets compacted away).
- `history/export.ts`: an image block renders as an embedded `![attached image](data:...)` in the Markdown export - a self-contained record someone can open and see exactly what was shown to the model, matching the audit intent from Phase 28.
- `server.ts`: `ClientMessage`'s `input` and `ServerMessage`'s `user_text` both gain an optional `images` array. The empty-send guard changed from "no text = no-op" to "no text *and* no images = no-op," so an image with no caption still sends.
- `web-app/`: `ChatInput.tsx` gets a 📎 button (hidden file input, `accept` limited to the four media types Anthropic actually supports - jpeg/png/gif/webp, since OpenRouter accepts more but there's no point letting someone attach something that breaks the moment they `/provider anthropic`) plus paste-an-image-from-clipboard support, both converting to base64 client-side via `FileReader`, with thumbnail previews and a per-attachment remove button before sending. `ChatFeed.tsx` and `useHarnessSocket.ts` (`renderHistory` and the live `user_text` handler) render an image block as an actual `<img>` bubble. The legacy vanilla client (`/legacy`) didn't gain upload UI (per the guide, that's Phase 10's client specifically) but did get a small view-only `addImage()` so a session fed an image from the React client still renders instead of silently vanishing when viewed there.

**Real bug found and fixed along the way, not from this phase's own new code:** live-testing the `PATCH /api/chats/:id` route from Phase 28 with a pin-only body crashed the server on a later export - `ChatHistoryStore.update()`'s `{ ...summary, ...changes }` doesn't skip an explicitly-`undefined` field the way `server.ts`'s route always sends (`{title: undefined, pinned: true}` for a pin-only request), so it silently wiped the title to `undefined`. Fixed by only applying a change when it's actually present, plus a regression test (`store.test.ts`) that mirrors the exact shape the route sends.

**Verified for real, not just typechecked:** 212 backend tests (5 new: 2 in `agent.test.ts` for the image-block-append and empty-prompt-with-image cases, 2 in `compactor.test.ts` for the token-counting/placeholder split, 1 regression test for the `update()` bug above), `npm run build` (web-app) clean. Live smoke test against the real server on both providers: generated a real 64×64 solid-red PNG with `ffmpeg`, sent it over a live WebSocket with a caption ("What color is this?") through OpenRouter - the model answered "Red" - then sent it again with *no* caption at all, and it still answered "Red," confirming the empty-prompt-plus-image path works end to end, not just past typecheck. Confirmed the chat archived both image blocks correctly (`GET /api/chats/:id/export?format=json`) and that the Markdown export embeds the data URI. Also switched to `/provider anthropic` mid-session and sent the same image - got a clean `invalid_request_error` about account credit balance (the same known-empty-credit account noted in Phase 25), which is exactly the signal that matters here: the request passed Anthropic's own request validation and only failed on billing, proving `toAnthropicBlock`'s image translation is well-formed against the real API, not a local guess.

Try it: click the 📎 button (or paste a screenshot) in the chat, ask "what's in this image?", and send.

## Phase 30: Document Attachments (PDFs, Floor Plans)

**Key concept:** a generalization of Phase 29 - not everything worth showing the model is a photo. A PDF (a signed contract, a window/door floor plan) needs its own handling: extract the text if there is any, or fall back to rendering pages as images (reusing Phase 29's `image` Block) if it's mostly visual. No Go module to migrate: another feature with no equivalent in `internal/api/types.go`.

- `pdf-parse` (new runtime dependency, v2): wraps `pdfjs-dist` internally and gives both text extraction (`getText()`) and real page rasterization (`getScreenshot()`) from one actively-maintained package - no separate `pdfjs-dist`/canvas dependency needed at this project's level, avoiding exactly the kind of heavy native-binary risk the Fase 39 memory note already flagged against for local embeddings.
- `documents/pdf.ts` (new): `extractPdfText()`, `looksMostlyVisual()` (a "mostly visual" PDF is one averaging under ~40 extracted characters per page - real text-based PDFs land nowhere close to that, a scanned/image-only PDF lands at ~0), `renderPdfPages()` (capped at 5 pages - a rendered page is a real image payload, tokens and bytes both), and `pdfToBlocks()` - the function server.ts's web-attachment path uses, returning either a single prefixed text `Block` (`[document: name.pdf, N page(s)]\n<text>`) or one `image` `Block` per rendered page.
- **A real architectural split, not an oversight:** a `Tool`'s `ToolResult.result` is a plain string (see `tools/types.ts`), and while Anthropic's `tool_result` blocks *can* carry images, OpenAI-compatible "tool" role messages (what OpenRouter uses) can't - and a `Tool` implementation has no idea which provider is active, by design. So `tools/read_document.ts` (new, same read-only tier as `read_file`) only ever returns extracted text; a "mostly visual" PDF read via the tool gets an honest message ("attach it directly in the chat instead") rather than silently working on one provider and not the other. The web-attachment path doesn't have this constraint - a PDF attached to a chat message becomes part of a user `Message`, where an `image` `Block` is valid on both providers, so *that* path really does render and show the pages.
- `agent.ts`: `send()`'s Phase 29 `images` parameter is generalized to `blocks: Block[]` - Phase 30 needed to append either image blocks (rendered pages) or a text block (extracted content) the exact same way, and a generic `Block[]` is simpler than two near-identical parameters.
- `server.ts`: `ClientMessage.input` gains `documents` (filename/mediaType/base64 data, PDFs only - anything else is dropped, same fail-closed spirit as the client's own mediaType allowlist). Each document is turned into blocks *before* anything is broadcast - a "mostly visual" PDF's rendered pages join the same `images` array a genuine photo attachment would use (a live viewer sees them exactly the same way), but the extracted text of a text-mode PDF is deliberately **not** re-broadcast live (it can be the whole document) - only a compact `📄 filename.pdf` chip goes out immediately (`ServerMessage.user_text.documents`, metadata only, no bytes), with the full text landing in `agent.getMessages()` - visible via reconnect/`history` and Phase 28's export, not duplicated into the live feed as a giant bubble.
- `tools.json` / `tools.readonly.json`: both gain `"read_document"` (client role and the readonly profile too - same read-only tier as `read_file`).
- `web-app/`: `ChatInput.tsx`'s attach button now accepts `application/pdf` alongside images, tracking two attachment kinds (image vs. document) with distinct previews (a thumbnail vs. a filename chip). `useHarnessSocket.ts` renders a document chip on `user_text` - no changes needed for `renderHistory` at all, since a document's derived content is just an ordinary `text`/`image` `Block` the Phase 29 renderer already understands. The legacy `/legacy` client got the same one-line chip for parity, for the same reason as Phase 29 (a session fed a PDF from the React client shouldn't look broken there).
- `documents/pdf.test.ts` / `tools/read_document.test.ts` (new): fixtures built on the fly with `pdf-lib` (a devDependency, not shipped with the harness) rather than checked-in binary PDFs - same "construct it, don't commit a blob" preference this project's config/skills tests already use.

**Verified for real, not just typechecked:** 224 backend tests (11 new), `npm run build` (web-app) clean. Live smoke test against the real server through OpenRouter, using two real PDFs (one built with LibreOffice containing real text and a planted secret code word, one a solid-red-square page with no text at all, both generated fresh, not committed to the repo): attaching the text PDF with a caption got a correct summary *and* the exact planted code word back; attaching the visual-only PDF with no caption made the model correctly describe "a solid red square... white background" - proof it actually saw the rendered page, not a hallucinated guess; and asking the agent to call `read_document` directly against the text PDF's path on disk returned the same code word through the tool-call path. Confirmed the chat-history archive captured both the derived text block and the derived image block correctly, and that the Markdown export contains both the `[document: ...]` marker and the embedded rendered-page image.
...
## Phase 33 (Opcional, cosmética): TUI Enriquecida

**Key concept:** visual polish only - no new capability, just a better terminal experience. Reordered ahead of Phases 34-38 (Tailscale/TLS/stricter-remote-permissions/PWA/notifications) at Eloy's request partway through planning them, since it's purely cosmetic and has zero dependency on any of the infrastructure work those phases need (a real VPS, Tailscale installed). Go module referenced by the guide: `internal/ui/{banner,highlight,styles}.go` (Bubble Tea + Lipgloss + `chroma`) - used as a reference for *what* to build, not literally ported, since this project's own TUI is already Ink-based, not Bubble Tea.

- `ui/styles.ts` (new): six tiny ANSI wrap-in-escape-codes helpers (`bold`, `dim`, `cyan`, `green`, `red`, `yellow`) - mirrors Go's minimal `styles.go` vocabulary rather than pulling in a color library for a handful of functions. Plain strings, not Ink `<Text>` - both call sites (the startup banner, printed before Ink even mounts, and the approval-prompt diff inside `App.tsx`) go through `console.log`, which Ink's own patch forwards to the scrollback unchanged either way.
- `ui/highlight.ts` (new): `highlightDiff()`, scoped down from Go's version on purpose - Go's `highlight.go` also color-detects JSON for its debug modal, but nothing in this TUI renders raw JSON to a human in a way that's worth a second detection path, so this stays diff-only. No highlighting library either (the guide names `cli-highlight`/`shiki` as candidates, asking to weigh dependency cost vs. benefit) - a unified diff's structure is four line prefixes (`+++`/`---` file headers, `@@` hunk headers, `+`/`-` changed lines), not a real language grammar, so a ~15-line function covers it without a dependency.
- `ui/banner.ts` (new): `bannerText(providerKind, model, toolNames)` - deliberately *not* a giant ASCII wordmark like Go's original. This boilerplate isn't a fixed-brand product (every deployment is meant to rename/reconfigure it via `harness.config.json`, Phase 8), so a small, tasteful header (a cyan rule, bold name, dim provider/model/tools line) reads better on something that gets forked per project - closer to Go's own narrow-terminal fallback than its full animated shine effect.
- `tui.tsx` / `App.tsx`: the startup banner replaces the three plain `console.log` lines that used to print model/tools/help text. The tool-call line (`[tool] name {...}`) is now dimmed and the risk-flag line is colored by severity (amber for `low`, red for `high`) - matching the same split the web UI's risk chip already uses (Phase 18), so the signal reads consistently across both surfaces. The approval prompt's diff (`buildWriteDiff`, Phase 11) now goes through `highlightDiff()` before printing - **TUI only**, per the guide's explicit scope; `index.ts`'s plain console REPL stays untouched on purpose (it's the deliberately "no magic" reference entry point, same reasoning Phase 6 already established for keeping a plain baseline next to the polished surface).

**Verified for real, not just typechecked:** 256 backend tests (10 new: 6 for `highlightDiff`'s four line-prefix cases, 2 for the color helpers, 2 for the banner's content). Live end-to-end check of the *actual interactive TUI* - not just the pure functions in isolation - using `pexpect` to drive a real pseudo-terminal session against `npx tsx src/tui.tsx`: captured the raw ANSI bytes and confirmed the banner rendered with the right codes (bold cyan header, dim subtitle/tools line, cyan rule), the tool-call line came through dimmed, and asking the agent to overwrite a real file produced a diff where the `---`/`+++` headers were dimmed, the `@@` hunk header was cyan, removed lines were red, and added lines were green - exactly the four cases the unit tests check, now confirmed against what actually prints to a real terminal.

Try it: `npm run tui`, compare the startup banner to before this phase, then ask the agent to overwrite an existing file and check the diff in the approval prompt renders with colors instead of plain text.
